...
game, one person asks a second person to fill in each of the placeholders without the second person knowing the overall story. Once all placeholders are filled in, the second person is shown the resulting story, often with humorous results. See https://en.wikipedia.org/wiki/Mad_Libs for more info.
...
```
Keep in mind that:
* your poem needs to be at least 10 lines in length
* you need to use all three tags (```noun```, ```verb```, ```adjective```)
* the mad lib needs to make sense syntactically (albeit it might be nonsensical), i.e. plural vs. singular, tense (past vs. present), etc.

The ```MadLibs/data``` already contains dictionaries of nouns, verbs and adjectives. Using these is optional and you are allowed to use other dictionaries if you want to. Each dictionary has to have at least 100 words in it.
